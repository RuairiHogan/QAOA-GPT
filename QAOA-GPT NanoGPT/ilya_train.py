...
from ilya_model import GPTConfig, GPT
...
train_data = np.load(
    os.path.join(data_dir, 'train_seqs.npy'), mmap_mode=mmap
)
val_data = np.load(
    os.path.join(data_dir, 'val_seqs.npy'), mmap_mode=mmap
)
train_graph_emb_np = np.load(
    os.path.join(data_dir, 'train_graph_emb.npy'), mmap_mode=mmap
)
val_graph_emb_np = np.load(
    os.path.join(data_dir, 'val_graph_emb.npy'), mmap_mode=mmap
)
emb_dim = train_graph_emb_np.shape[1]

def get_batch(split):

    if split == 'train':
        data = train_data
        emb_data = train_graph_emb_np
    else:
        data = val_data
        emb_data = val_graph_emb_np

    ix = np.random.randint(low=0, high=data.shape[0], size=batch_size)  # include last row too
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if losses['val'] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                ...
                torch.save(checkpoint, os.path.join(out_dir, f'ckpt_{i}.pt'))
