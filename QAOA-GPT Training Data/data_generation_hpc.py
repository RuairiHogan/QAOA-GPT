...
MAX_DEPTH = 9
GAMMA0_GRID = [0.01, 0.1, 0.3, 0.5, 0.8, 1.0]  # np.linspace(0.01, 1.0, 10)
TARGET_AR = 0.97

# Default output filename (can be overridden by command-line argument)
DEFAULT_DATASET_FILE = "qaoa_gpt_dataset_connectivity.jsonl"
...
# Number of best candidates to keep per graph
TOP_K = 1

# "PQAOA" -> HB only
# "PDUAL" -> single + 2-qubit Pauli strings
OP_POOL_MODE = "PDUAL"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate a QAOA GPT dataset and write it to a JSONL file."
    )
    parser.add_argument(
        "--output",
        "-o",
        default=DEFAULT_DATASET_FILE,
        help=f"Output JSONL filename (default: {DEFAULT_DATASET_FILE})"
    )
