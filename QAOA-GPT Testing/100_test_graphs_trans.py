...
INPUT_FILE = "generated_circuits_trans_100x10.txt"
OUTPUT_JSON = "ideal_simulation_results_trans_100x10.json"
OUTPUT_PLOT_INDEX = "ideal_simulation_trans_best_vs_avg_ar_by_graph.png"
OUTPUT_PLOT_SORTED = "ideal_simulation_trans_best_vs_avg_ar_sorted.png"
OUTPUT_PLOT_SCATTER = "ideal_simulation_trans_best_vs_avg_ar_scatter.png"
...
########################################
# OPERATOR POOL (FULL PDUAL ORDER)
########################################
...
def parse_tokens(tokens):
    ...
    if i >= len(tokens) or tokens[i] != "<bos>":
        raise ValueError("Expected <bos> near start of line")
    i += 1

    if i >= len(tokens) or tokens[i] != "<maxcut_graph>":
        raise ValueError(
            "Expected <maxcut_graph>. This file looks like graph-only trans data, "
            "not trans circuits."
        )
    i += 1

    edges = []
    while i < len(tokens) and tokens[i] != "<end_of_maxcut_graph>":
        if i + 1 >= len(tokens):
            raise ValueError("Incomplete edge-weight pair in graph section")
        u_str, v_str = tokens[i].strip("()").split(",")
        w = float(tokens[i + 1])
        edges.append((int(u_str), int(v_str), w))
        i += 2

    if i >= len(tokens) or tokens[i] != "<end_of_maxcut_graph>":
        raise ValueError("Missing <end_of_maxcut_graph>")
    i += 1

    if i >= len(tokens) or tokens[i] != "<circuit>":
        raise ValueError(
            "Missing <circuit>. The input file does not contain transpiled circuits."
        )
    i += 1
    ...
    while i < len(tokens) and tokens[i] != "<end_of_circuit>":
        if not tokens[i].startswith("<new_layer_"):
            raise ValueError(f"Unexpected token in circuit section: {tokens[i]}")
        if i + 3 >= len(tokens):
            raise ValueError(f"Incomplete layer block starting at {tokens[i]}")
...
def style_axis(ax, ymin, ymax):
    ax.set_xlabel("Graph index", fontsize=LABEL_FONT_SIZE)
    ax.set_ylabel("Approximation ratio", fontsize=LABEL_FONT_SIZE)
...
def make_scatter_plot(best_ars, avg_ars, output_plot):
    ...
    ax.scatter(x, best_ars, s=28, alpha=0.8, marker="o", label="Best AR of 10 samples")
    ax.scatter(x, avg_ars, s=28, alpha=0.8, marker="s", label="Average AR of 10 samples")
    ...
    style_axis(ax, ymin, ymax)
    ax.set_xlim(1, len(best_ars))
    ax.legend(loc="lower left", frameon=True, fontsize=LEGEND_FONT_SIZE)
